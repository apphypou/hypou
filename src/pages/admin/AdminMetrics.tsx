...
const AdminMetrics = () => {
  ...
  const sections = [
    ...
    ["Engajamento", [["DAU", number(validation.engagement.dau), "Usuários com sessão nas últimas 24 horas."], ["WAU", number(validation.engagement.wau), "Usuários ativos nos últimos 7 dias."], ["MAU", number(validation.engagement.mau), "Usuários ativos nos últimos 30 dias."], ["Frequência de acesso", validation.engagement.accessFrequency === null ? "Aguardando dados" : `${validation.engagement.accessFrequency.toLocaleString("pt-BR")} sessões/usuário`, "Média de sessões por usuário ativo nos últimos 30 dias."], ["Interações registradas", number(validation.engagement.interactions), "Eventos de produto no período medido."]]],
    ...
    ["Satisfação", [["Avaliação média", validation.satisfaction.averageRating?.toFixed(1).replace(".", ",") || "Sem avaliações", "Média das avaliações pós-troca."], ["NPS", validation.satisfaction.nps === null ? "Sem respostas" : String(validation.satisfaction.nps), "Promotores menos detratores."], ["Promotores", number(validation.satisfaction.npsPromoters), "Pessoas com nota 9 ou 10, com intenção de recomendar."], ["Respostas NPS", number(validation.satisfaction.npsResponses), "Respostas recebidas nos últimos 90 dias."]]],
