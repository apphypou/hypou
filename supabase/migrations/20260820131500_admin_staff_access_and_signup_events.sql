-- Staff visibility is intentionally explicit. Operational mutation stays
-- constrained to the UI/RPC flows instead of exposing public write access.
...
CREATE POLICY "Staff can view match items"
...
BEGIN
  ...
  VALUES (NEW.user_id, 'signup_completed', '{}'::jsonb);
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS record_profile_signup_event ON public.profiles;
CREATE TRIGGER record_profile_signup_event
