-- Completes the first operational slice of the admin panel: reliable product
-- analytics, staff access and platform-level suspensions.
...
CREATE INDEX IF NOT EXISTS product_events_event_user_occurred_at_idx
  ON public.product_events (event_name, user_id, occurred_at DESC)
  WHERE user_id IS NOT NULL;

CREATE POLICY "Users can record own acquisition attribution"
...
CREATE INDEX user_suspensions_active_idx
  ON public.user_suspensions (suspended_at DESC)
  WHERE lifted_at IS NULL;

ALTER TABLE public.user_suspensions ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Staff can view suspensions"
...
CREATE OR REPLACE FUNCTION public.reject_suspended_user_action()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE TRIGGER reject_suspended_item_write
...
-- Moderators can work through the same read surfaces as administrators, while
-- membership and launch configuration stay restricted to administrators.
...
CREATE POLICY "Staff can view matches"
...
CREATE POLICY "Staff can view beta testers"
  ON public.beta_testers FOR SELECT TO authenticated
  USING (public.is_staff(auth.uid()));

CREATE POLICY "Staff can update beta testers"
...
CREATE POLICY "Staff can view incidents"
  ON public.system_incidents FOR SELECT TO authenticated
  USING (public.is_staff(auth.uid()));

CREATE POLICY "Staff can view uptime checks"
  ON public.uptime_checks FOR SELECT TO authenticated
  USING (public.is_staff(auth.uid()));

CREATE POLICY "Staff can view product events"
  ...
  IF p_suspended THEN
    ...
    ON CONFLICT (user_id) DO UPDATE
      SET reason = EXCLUDED.reason,
          ...
          lifted_at = NULL,
          lifted_by = NULL;
    INSERT INTO public.admin_audit_logs (actor_id, action, target_id, metadata)
    VALUES (auth.uid(), 'user_suspended', p_user_id, jsonb_build_object('reason', normalized_reason));
  ELSE
    UPDATE public.user_suspensions
      SET lifted_at = now(), lifted_by = auth.uid()
      WHERE user_id = p_user_id AND lifted_at IS NULL;
    INSERT INTO public.admin_audit_logs (actor_id, action, target_id)
    VALUES (auth.uid(), 'user_unsuspended', p_user_id);
  END IF;
END;
$$;
